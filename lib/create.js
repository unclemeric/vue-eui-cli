...
module.exports = async function (rawName, dirTemplates, options) {
  ...
  if (fs.existsSync(to)) {
    inquirer
      ...
      .then(answers => {
        if (answers.continue) {
          fileUtil.generate(dirTemplates, to, projectNam, options)
    ...
    fileUtil.generate(dirTemplates, to, projectName, options)
